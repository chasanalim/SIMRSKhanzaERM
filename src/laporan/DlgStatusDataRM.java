...
public final class DlgStatusDataRM extends javax.swing.JDialog {
    ...
    private int i=0,adasoapiralan=0,tidakadasoapiralan=0,adasoapiranap=0,tidakadasoapiranap=0,adaresumeralan=0,tidakadaresumeralan=0,
            ...
            adaicd9=0,tidakadaicd9=0,adaaskepranap=0,tidakadaaskepranap=0,adaasmedranap=0,tidakadaasmedranap=0,adaperencanaanpulang=0,
            tidakadaperencanaanpulang=0,adaverifsbar=0,tidakadaverifsbar=0,adadpjp=0,tidakadadpjp=0,adaresikojatuh=0,tidakadaresikojatuh=0;  
    private String soapiralan="",soapiranap="",resumeralan="",resumeranap="",pilihan="",triaseigd="",askepigd="",icd10="",icd9="",askepranap="",asmedranap="",perencanaanpulang="",verifsbar="",dpjp="",resikojatuh="";
    ...
    public DlgStatusDataRM(java.awt.Frame parent, boolean modal) {
        ...
        tabMode=new DefaultTableModel(null,new Object[]{
            "No.Rawat","Tanggal","Dokter Dituju","Nomer RM","Pasien","Poliklinik","Status","SOAP Ralan","SOAP Ranap","Resume Ralan","Resume Ranap","Triase IGD","Askep IGD","ICD 10","ICD 9",
            "Askep Ranap","Asmed Ranap","Perencanaan Pulang","Verf SBAR","DPJP","Resiko Jatuh"
        }){
    ...
    public void tampil(){        
        try{   
            ...
            ps=koneksi.prepareStatement(
                ...
                "inner join poliklinik on reg_periksa.kd_poli=poliklinik.kd_poli inner join penjab on reg_periksa.kd_pj=penjab.kd_pj where reg_periksa.stts <> 'Batal' and "+
            ...
            try {
                ...
                adaaskepranap=0;tidakadaaskepranap=0;adaasmedranap=0;tidakadaasmedranap=0;adaperencanaanpulang=0;
                tidakadaperencanaanpulang=0;adaverifsbar=0;tidakadaverifsbar=0;adadpjp=0;tidakadadpjp=0;adaresikojatuh=0;tidakadaresikojatuh=0; 
                while(rs.next()){
                    //SOAP RALAN
                    ...
                    //ASKEP RANAP
                    askepranap=Sequel.cariIsi3("SELECT IF ( sum( jumlah ) > 0, 'Ada', 'Tidak Ada' ) FROM (SELECT count( penilaian_awal_keperawatan_ranap.no_rawat ) AS jumlah FROM penilaian_awal_keperawatan_ranap WHERE "+ 
                                              "penilaian_awal_keperawatan_ranap.no_rawat = ? UNION SELECT count( penilaian_awal_keperawatan_kebidanan_ranap.no_rawat ) AS jumlah FROM penilaian_awal_keperawatan_kebidanan_ranap WHERE penilaian_awal_keperawatan_kebidanan_ranap.no_rawat = ? ) AS askep_ranap",rs.getString("no_rawat"),rs.getString("no_rawat"));
                    if(askepranap.equals("Ada")){
                        adaaskepranap++;
                    }else{
                        tidakadaaskepranap++;
                    }
                    //ASMED RANAP
                    asmedranap=Sequel.cariIsi3("SELECT IF ( sum( jumlah ) > 0, 'Ada', 'Tidak Ada' ) FROM (SELECT count( penilaian_medis_ranap.no_rawat ) AS jumlah FROM penilaian_medis_ranap WHERE "+ 
                                              "penilaian_medis_ranap.no_rawat = ? UNION SELECT count( penilaian_medis_ranap_kandungan.no_rawat ) AS jumlah FROM penilaian_medis_ranap_kandungan WHERE penilaian_medis_ranap_kandungan.no_rawat = ? ) AS asmed_ranap",rs.getString("no_rawat"),rs.getString("no_rawat"));
                    if(asmedranap.equals("Ada")){
                        adaasmedranap++;
                    }else{
                        tidakadaasmedranap++;
                    }
                    //PERENCANAAN PULANG
                    perencanaanpulang=Sequel.cariIsi("select if(count(perencanaan_pemulangan.no_rawat)>0,'Ada','Tidak Ada') from perencanaan_pemulangan where perencanaan_pemulangan.no_rawat=?",rs.getString("no_rawat"));
                    if(perencanaanpulang.equals("Ada")){
                        adaperencanaanpulang++;
                    }else{
                        tidakadaperencanaanpulang++;
                    }
                    //VERIF SBAR
                    verifsbar=Sequel.cariIsi("select if(count(validasi_pemeriksaan_sbar.no_rawat)>0,'Ada','Tidak Ada') from validasi_pemeriksaan_sbar where validasi_pemeriksaan_sbar.no_rawat=?",rs.getString("no_rawat"));
                    if(verifsbar.equals("Ada")){
                        adaverifsbar++;
                    }else{
                        tidakadaverifsbar++;
                    }
                    //DPJP
                    dpjp=Sequel.cariIsi("select if(count(dpjp_ranap.no_rawat)>0,'Ada','Tidak Ada') from dpjp_ranap where dpjp_ranap.no_rawat=?",rs.getString("no_rawat"));
                    if(dpjp.equals("Ada")){
                        adadpjp++;
                    }else{
                        tidakadadpjp++;
                    ...
                    //RESIKOJATUH
                    resikojatuh=Sequel.cariIsi4("SELECT IF ( sum( jumlah ) > 0, 'Ada', 'Tidak Ada' ) FROM (SELECT count( penilaian_lanjutan_resiko_jatuh_anak.no_rawat ) AS jumlah FROM penilaian_lanjutan_resiko_jatuh_anak WHERE "+
                                                "penilaian_lanjutan_resiko_jatuh_anak.no_rawat = '2023/07/16/000025' UNION SELECT count( penilaian_lanjutan_resiko_jatuh_dewasa.no_rawat ) AS jumlah FROM "+ 
                                                "penilaian_lanjutan_resiko_jatuh_dewasa WHERE penilaian_lanjutan_resiko_jatuh_dewasa.no_rawat = '2023/07/16/000025' UNION SELECT count( penilaian_lanjutan_resiko_jatuh_lansia.no_rawat ) AS jumlah FROM "+
                                                "penilaian_lanjutan_resiko_jatuh_lansia WHERE penilaian_lanjutan_resiko_jatuh_lansia.no_rawat = '2023/07/16/000025' UNION SELECT count( penilaian_lanjutan_resiko_jatuh_geriatri.no_rawat ) AS jumlah FROM "+
                                                "penilaian_lanjutan_resiko_jatuh_geriatri WHERE penilaian_lanjutan_resiko_jatuh_geriatri.no_rawat = '2023/07/16/000025') AS asmed_ranap",rs.getString("no_rawat"),rs.getString("no_rawat"),rs.getString("no_rawat"),rs.getString("no_rawat"));
                    if(resikojatuh.equals("Ada")){
                        adaresikojatuh++;
                    }else{
                        tidakadaresikojatuh++;
                    }
                    tabMode.addRow(new Object[]{
                        ...
                        soapiralan,soapiranap,resumeralan,resumeranap,triaseigd,askepigd,icd10,icd9,askepranap,askepranap,perencanaanpulang,verifsbar,dpjp,resikojatuh
                    });                    
                }
                if(tabMode.getRowCount()>0){
                    tabMode.addRow(new Object[]{
                        "","","","","","Status Data Ada",":",adasoapiralan,adasoapiranap,adaresumeralan,adaresumeranap,adatriaseigd,adaaskepigd,adaicd10,adaicd9,adaaskepranap,adaasmedranap,adaperencanaanpulang,adaverifsbar,adadpjp,adaresikojatuh
                    });
                    tabMode.addRow(new Object[]{
                        "","","","","","Status Data Tidak Ada",":",tidakadasoapiralan,tidakadasoapiranap,tidakadaresumeralan,tidakadaresumeranap,tidakadatriaseigd,tidakadaaskepigd,tidakadaicd10,tidakadaicd9,tidakadaaskepranap,tidakadaasmedranap,tidakadaperencanaanpulang,tidakadaverifsbar,tidakadadpjp,tidakadaresikojatuh
                    });
                }   
            } catch (Exception e) {
